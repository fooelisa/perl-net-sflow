...
sub decode {
  ...
  # unknown ip version added in v5 
    ...
    $error = "ERROR: Unknown angent ip version\n";
		return (\%sFlowDatagram, \@sFlowSamples, \$error);
  }

  else { 
    $error = "ERROR: Wrong angent ip version\n";
		return (\%sFlowDatagram, \@sFlowSamples, \$error); 
  }
    ...
    for ($samplesCount = 0; $samplesCount < $sFlowDatagram{samplesInPacket}; $samplesCount++) {
      ...
      if ($sFlowSample{sampleType} == FLOWSAMPLE_SFLOWv4) {
        ...
        # packet data type: IPv6
          ...
          $error = "ERROR: <sFlowV4:PacketData> Wrong packet data type\n";
		      return (\%sFlowDatagram, \@sFlowSamples, \$error); 
        }
        ...
        for ($extendedDataCount = 0; $extendedDataCount < $sFlowSample{extendedDataInSample}; $extendedDataCount++) {
          ...
          # added in v.3.
            ...
            $error = "ERROR: <sFlowV4:ExtendedData> Wrong extended data type\n";
		        return (\%sFlowDatagram, \@sFlowSamples, \$error); 
          }
      ...
      # COUNTERSAMPLE
        ...
        # counterstype: vlan
          ...
          $error = "ERROR: <sFlowV4:CountersType> Wrong counters type\n";
		      return (\%sFlowDatagram, \@sFlowSamples, \$error); 
        }
        ...
        $error = "ERROR: <sFlowV4:SampleType> Wrong sample type\n";
		    return (\%sFlowDatagram, \@sFlowSamples, \$error); 
      }
...
####### sFlow V5 #######
    ...
    for ($samplesCount = 0; $samplesCount < $sFlowDatagram{samplesInPacket}; $samplesCount++) {
      ...
      if ($sFlowSample{sampleTypeEnterprise} == 0 and $sFlowSample{sampleTypeFormat} == FLOWSAMPLE_SFLOWv5) {
        ...
        $error = "ERROR: <sFlowV5:SampleData> Unknown sample enterprise or format\n";
		    return (\%sFlowDatagram, \@sFlowSamples, \$error); 
      }
    ...
    $error = "ERROR: Wrong sFlow Version\n";
		return (\%sFlowDatagram, \@sFlowSamples, \$error); 
  }
...
sub _decodeFlowRecord {
  ...
  if ($flowTypeEnterprise == 0) {
    ...
    if ($flowTypeFormat == HEADERDATA_SFLOWv5) {
      ...
      $error = "ERROR: <sFlowV5:FlowData> Unknown Flowdata format\n";
		  return ($sFlowDatagram, $sFlowSamples, \$error); 
    }
    ...
    $error = "ERROR: <sFlowV5:FlowData> Unknown Flowdata enterprise\n";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
}
...
sub _decodeCounterRecord {
  ...
  if ($counterTypeEnterprise == 0) {
    ...
    if ($counterTypeFormat == GENERICCOUNTER_SFLOWv5) {
      ...
      $error = "ERROR: <sFlowV5:CounterData> Unknown counterdata format\n";
		  return ($sFlowDatagram, $sFlowSamples, \$error); 
    }
    ...
    $error = "ERROR: <sFlowV5:CounterData> Unknown counterdata enterprise\n";
	  return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
...
sub _decodeHeaderData {
  ...
  if ((exists($ipObj->{proto}) and $ipObj->{proto} == 6) or (exists($ipObj->{nexth}) and $ipObj->{nexth} == 6)) {
    ...
    my $error = "ERROR: <HeaderDtata> Unknown next header protocol\n";
    return ($sFlowDatagram, $sFlowSamples, \$error);
  } 
...
sub _decodeRouterData {
  ...
  if ($sFlowSample->{RouterIpVersionNextHopRouter} == IPv4) {
    ...
    my $error = "ERROR: <RouterData> Unknown ip version next hop router\n";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
...
sub _decodeGatewayData {
  ...
  if ($sFlowDatagram->{sFlowVersion} == SFLOWv5) {
    ...
    if ($sFlowSample->{GatewayIpVersionNextHopRouter} == IPv4) {
      ...
      my $error = "ERROR: <GatewayData> Unknown ip version next hop router\n";
		  return ($sFlowDatagram, $sFlowSamples, \$error); 
    }
...
sub _decodeMplsData {
  ...
  if ($sFlowSample->{MplsIpVersionNextHopRouter} == IPv4) {
    ...
    my $error = "ERROR: <MplsData> Unknown ip version next hop router\n";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
...
sub _decodeNatData {
  ...
  if ($sFlowSample->{NatIpVersionSrcAddress} == IPv4) {
    ...
    $error = "ERROR: <NatData> Unknown ip version src address\n";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
  ...
  if ($sFlowSample->{NatIpVersionDestAddress} == IPv4) {
    ...
    $error = "ERROR: <NatData> Unknown ip version dest address\n";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
