...
# Tags:
    ...
    versions    => [qw(IP_VERSION_IPv4 IP_VERSION_IPv6)],
    strip       => [qw(ip_strip)],
...
# Elisa Jasinska <elisa@ams-ix.net>, 2006/07/10
#

use constant IP_IPV6_EXT_HBH        => 0;     # Hop-by-Hop Options Header
use constant IP_IPV6_EXT_ROUTING    => 43;    # Routing Header
use constant IP_IPV6_EXT_FRAGMENT   => 44;    # Fragment Header
use constant IP_IPV6_EXT_ESP        => 50;    # Encapsulating Security Payload Header 
use constant IP_IPV6_EXT_AH         => 51;    # Authentication Header
use constant IP_IPV6_EXT_NNH        => 59;    # No Next Header
use constant IP_IPV6_EXT_DEST       => 60;    # Destination Options Header
...
#
# if statement added to decide whether IPv4 or IPv6 should be decoded
# 2006/07/10 Elisa Jasinska <elisa@ams-ix.net>
#

      if ($self->{ver} == IP_VERSION_IPv4){
...
#
# IPv6 decode 
# added by Elisa Jasinska <elisa@ams-ix.net>, 2006/07/10
#

      elsif ($self->{ver} == IP_VERSION_IPv6) {

        ($tmp, $self->{tclass}, $self->{flabel},
        $self->{datalen}) = unpack('B4CB20n', $pkt);

        $pkt = substr($pkt, 6);
  
        ($self->{nexth}, $self->{hlim}, $self->{src_ip},
        $self->{dest_ip}) = unpack("CCB128B128", $pkt);
        ...
        while (1) {
          ...
          if ($self->{nexth} == IP_IPV6_EXT_HBH ||
              $self->{nexth} == IP_IPV6_EXT_ROUTING || 
              $self->{nexth} == IP_IPV6_EXT_DEST) {

            ($self->{nexth}, $self->{exlen}) = unpack('CC',$pkt);
            my $cut = 8 + ($self->{exlen} * 8);
            $pkt = substr($pkt,$cut);

          }

          elsif ($self->{nexth} == IP_IPV6_EXT_AH) {

            ($self->{nexth}, $self->{exlen}) = unpack('CC',$pkt);
            my $cut = 8 + ($self->{exlen} * 4);
            $pkt = substr($pkt,$cut);

          }

          elsif ($self->{nexth} == IP_IPV6_EXT_FRAGMENT) {

            # get 32bit
            $tmp = unpack('N',$pkt);
            # cut off 64bit (length of frangemt header)
            $pkt = substr($pkt, 8);
            # first 8bit next header
            $self->{nexth} = vec($tmp, 0, 8);

            # if the fragment is not the first (offset=0) drop the package
        
            if (($tmp >> 3 & 2 ** 13 - 1) == 0) {
              $self->{nexth} = IP_IPV6_EXT_NNH;
            }

          }

          # encrypted payload
          elsif ($self->{nexth} == IP_IPV6_EXT_ESP) { 
            $self->{nexth} = IP_IPV6_EXT_NNH;
          }
        ...
        # no transport layer data
        if ($self->{nexth} == IP_IPV6_EXT_NNH) { 
          $self->{data} = undef; 
        } 
