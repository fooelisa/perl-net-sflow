...
sub decode {
  ...
  # agent ip v4
  if ($sFlowDatagram{AgentIpVersion} == IPv4) {
    &_decodeAgentIPv4(\$sFlowDatagramPacked, \%sFlowDatagram);
  } 

  # agent ip v6
  elsif ($sFlowDatagram{AgentIpVersion} == IPv6) {
    &_decodeAgentIPv4(\$sFlowDatagramPacked, \%sFlowDatagram);
  }

  # unknown ip version added in v5 
  elsif ($sFlowDatagram{AgentIpVersion} == UNKNOWNIPVERSION) {
    $error = "ERROR: Unknown angent ip version: $sFlowDatagram{AgentIpVersion}";
		return (\%sFlowDatagram, \@sFlowSamples, \$error);
  }
    ...
    for ($samplesCount = 0; $samplesCount < $sFlowDatagram{samplesInPacket}; $samplesCount++) {
      ...
      if ($sFlowSample{sampleType} == FLOWSAMPLE_SFLOWv4) {
        ...
        if ($sFlowSample{packetDataType} == HEADERDATA_SFLOWv4) {
          &_decodeHeaderData(\$sFlowDatagramPacked, \%sFlowDatagram, \%sFlowSample, \@sFlowSamples); 
        }
        ...
        # packet data type: IPv6
          ...
          $error = "ERROR: <sFlowV4:PacketData> Unknown packet data type: $sFlowSample{packetDataType}";
		      return (\%sFlowDatagram, \@sFlowSamples, \$error); 
        }
        ...
        for ($extendedDataCount = 0; $extendedDataCount < $sFlowSample{extendedDataInSample}; $extendedDataCount++) {
          ...
          # extended data: router
            ...
            &_decodeRouterData(\$sFlowDatagramPacked, \%sFlowDatagram, \%sFlowSample, \@sFlowSamples);
          }

          # extended data: gateway
          elsif ($extendedDataType == GATEWAYDATA_SFLOWv4) {
            &_decodeGatewayData(\$sFlowDatagramPacked, \%sFlowDatagram, \%sFlowSample, \@sFlowSamples);
          }
          ...
          # added in v.3.
            ...
            $error = "ERROR: <sFlowV4:ExtendedData> Unknown extended data type: $extendedDataType";
		        return (\%sFlowDatagram, \@sFlowSamples, \$error); 
          }
      ...
      # COUNTERSAMPLE
        ...
        # counterstype: vlan
          ...
          $error = "ERROR: <sFlowV4:CountersType> Unknown counters type: $sFlowSample{countersVersion}";
		      return (\%sFlowDatagram, \@sFlowSamples, \$error); 
        }
        ...
        $error = "ERROR: <sFlowV4:SampleType> Unknown sample type: $sFlowSample{sampleType}";
		    return (\%sFlowDatagram, \@sFlowSamples, \$error); 
      }
...
####### sFlow V5 #######
    ...
    for ($samplesCount = 0; $samplesCount < $sFlowDatagram{samplesInPacket}; $samplesCount++) {
      ...
      if ($sFlowSample{sampleTypeEnterprise} == 0 and $sFlowSample{sampleTypeFormat} == FLOWSAMPLE_SFLOWv5) {
        ...
        for ($flowRecords = 0; $flowRecords < $sFlowSample{flowRecordsCount}; $flowRecords++) {
          &_decodeFlowRecord(\$sFlowDatagramPacked, \%sFlowDatagram, \%sFlowSample, \@sFlowSamples);
        }
        ...
        for ($counterRecords = 0; $counterRecords < $sFlowSample{counterRecordsCount}; $counterRecords++) {
          &_decodeCounterRecord(\$sFlowDatagramPacked, \%sFlowDatagram, \%sFlowSample, \@sFlowSamples);
        }
        ...
        $error = "ERROR: <sFlowV5:SampleData> Unknown sample enterprise: $sFlowSample{sampleTypeEnterprise} or format: $sFlowSample{sampleTypeFormat}";
		    return (\%sFlowDatagram, \@sFlowSamples, \$error); 
      }
    ...
    $error = "ERROR: Unknown sFlow Version: $sFlowDatagram{sFlowVersion}";
		return (\%sFlowDatagram, \@sFlowSamples, \$error); 
  }

  return (\%sFlowDatagram, \@sFlowSamples);
...
sub _decodeAgentIPv4 {
  ...
  ($sFlowDatagram->{AgentIp}) = unpack("B32", $$sFlowDatagramPacked);

  # cinvert bitstring to right notation
  $sFlowDatagram->{AgentIp} = Net::IP::ip_bintoip($sFlowDatagram->{AgentIp},4);

  # get rid of the 4 decoded bytes
  $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 4);
}
...
sub _decodeAgentIPv6 {
  ...
  ($sFlowDatagram->{AgentIp}) = unpack("B128", $$sFlowDatagramPacked);

  # convert bitstring to dottet notation
  $sFlowDatagram->{AgentIp} = Net::IP::ip_bintoip($sFlowDatagram->{AgentIp},6);
  
  # cut the decoded bytes
  $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 16);
}
...
sub _decodeFlowRecord {
  ...
  if ($flowTypeEnterprise == 0) {
    ...
    if ($flowTypeFormat == HEADERDATA_SFLOWv5) {
      ...
      &_decodGatewayData($sFlowDatagramPacked, $sFlowDatagram, $sFlowSample, $sFlowSamples);
    } 
      ...
      &_decodeMplsData($sFlowDatagramPacked, $sFlowDatagram, $sFlowSample, $sFlowSamples);
    } 

    elsif ($flowTypeFormat == NATDATA_SFLOWv5) {
      &_decodeNatData($sFlowDatagramPacked, $sFlowDatagram, $sFlowSample, $sFlowSamples);
    } 
      ...
      $error = "ERROR: <sFlowV5:FlowData> Unknown Flowdata format: $flowTypeFormat";
		  return ($sFlowDatagram, $sFlowSamples, \$error); 
    }
    ...
    $error = "ERROR: <sFlowV5:FlowData> Unknown Flowdata enterprise: $flowTypeEnterprise";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
}
...
sub _decodeCounterRecord {
  ...
  if ($counterTypeEnterprise == 0) {
    ...
    if ($counterTypeFormat == GENERICCOUNTER_SFLOWv5) {
      ...
      $error = "ERROR: <sFlowV5:CounterData> Unknown counterdata format: $counterTypeFormat";
		  return ($sFlowDatagram, $sFlowSamples, \$error); 
    }
    ...
    $error = "ERROR: <sFlowV5:CounterData> Unknown counterdata enterprise: $counterTypeEnterprise";
	  return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
...
sub _decodeHeaderData {
  ...
  $sFlowSample->{HeaderNhead} = $ipObj->{nhead};
  ...
  if ((exists($ipObj->{proto}) and $ipObj->{proto} == 6) or (exists($ipObj->{nexth}) and $ipObj->{nexth} == 6)) {
    ...
    my $error = "ERROR: <HeaderDtata> Unknown next header protocol: $proto";
    return ($sFlowDatagram, $sFlowSamples, \$error);
  } 
...
sub _decodeEthernetFrameData {
  ...
  ($sFlowSample->{EtherMacPacketlength},
   ...
   $EtherDestMac1,
   $EtherDestMac2,
   $sFlowSample->{EtherPackettype}) = unpack("NNnNnN", $$sFlowDatagramPacked);
...
sub _decodeRouterData {
  ...
  if ($sFlowSample->{RouterIpVersionNextHopRouter} == IPv4) {
    ($sFlowSample->{RouterIpAddressNextHopRouter}) = unpack("B32", $$sFlowDatagramPacked);
    $sFlowSample->{RouterIpAddressNextHopRouter} = Net::IP::ip_bintoip($sFlowSample->{RouterIpAddressNextHopRouter},4);
    $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 4);
  } 

  elsif ($sFlowSample->{RouterIpVersionNextHopRouter} == IPv6) {
    ($sFlowSample->{RouterIpAddressNextHopRouter}) = unpack("B128", $$sFlowDatagramPacked);
    $sFlowSample->{RouterIpAddressNextHopRouter} = Net::IP::ip_bintoip($sFlowSample->{RouterIpAddressNextHopRouter},6);
    $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 16);
  } 
    
  else { 
    my $error = "ERROR: <RouterData> Unknown ip version next hop router: $sFlowSample->{RouterIpVersionNextHopRouter}";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
...
sub _decodeGatewayData {
  ...
  if ($sFlowDatagram->{sFlowVersion} == SFLOWv5) {
    ...
    if ($sFlowSample->{GatewayIpVersionNextHopRouter} == IPv4) {
      ($sFlowSample->{GatewayIpAddressNextHopRouter}) = unpack("B32", $$sFlowDatagramPacked);
      $sFlowSample->{GatewayIpAddressNextHopRouter} = Net::IP::ip_bintoip($sFlowSample->{GatewayIpAddressNextHopRouter},4);
      $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 4);
    } 
  
    elsif ($sFlowSample->{GatewayIpVersionNextHopRouter} == IPv6) {
      ($sFlowSample->{GatewayIpAddressNextHopRouter}) = unpack("B128", $$sFlowDatagramPacked);
      $sFlowSample->{GatewayIpAddressNextHopRouter} = Net::IP::ip_bintoip($sFlowSample->{GatewayIpAddressNextHopRouter},6);
      $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 16);
    } 

    else { 
      my $error = "ERROR: <GatewayData> Unknown ip version next hop router: $sFlowSample->{GatewayIpVersionNextHopRouter}";
		  return ($sFlowDatagram, $sFlowSamples, \$error); 
    }
...
sub _decodeMplsData {
  ...
  if ($sFlowSample->{MplsIpVersionNextHopRouter} == IPv4) {
    ($sFlowSample->{MplsIpAddressNextHopRouter}) = unpack("B32", $$sFlowDatagramPacked);
    $sFlowSample->{MplsIpAddressNextHopRouter} = Net::IP::ip_bintoip($sFlowSample->{MplsIpAddressNextHopRouter},4);
    $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 4);
  } 
  
  elsif ($sFlowSample->{MplsIpVersionNextHopRouter} == IPv6) {
    ($sFlowSample->{MplsIpAddressNextHopRouter}) = unpack("B128", $$sFlowDatagramPacked);
    $sFlowSample->{MplsIpAddressNextHopRouter} = Net::IP::ip_bintoip($sFlowSample->{MplsIpAddressNextHopRouter},6);
    $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 16);
  } 
 
  else { 
    my $error = "ERROR: <MplsData> Unknown ip version next hop router: $sFlowSample->{MplsIpVersionNextHopRouter}";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
...
sub _decodeNatData {
  ...
  if ($sFlowSample->{NatIpVersionSrcAddress} == IPv4) {
    ($sFlowSample->{NatSrcAddress}) = unpack("B32", $$sFlowDatagramPacked);
    $sFlowSample->{NatSrcAddress} = Net::IP::ip_bintoip($sFlowSample->{NatSrcAddress},4);
    $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 4);
  }
  
  elsif ($sFlowSample->{NatIpVersionSrcAddress} == IPv6) {
    ($sFlowSample->{NatSrcAddress}) = unpack("B128", $$sFlowDatagramPacked);
    $sFlowSample->{NatSrcAddress} = Net::IP::ip_bintoip($sFlowSample->{NatSrcAddress},6);
    $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 16);
  }
 
  else { 
    $error = "ERROR: <NatData> Unknown ip version src address: $sFlowSample->{NatIpVersionSrcAddress}";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
  ...
  if ($sFlowSample->{NatIpVersionDestAddress} == IPv4) {
    ($sFlowSample->{NatDestAddress}) = unpack("B32", $$sFlowDatagramPacked);
    $sFlowSample->{NatDestAddress} = Net::IP::ip_bintoip($sFlowSample->{NatDestAddress},4);
    $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 4);
  }
  
  elsif ($sFlowSample->{NatIpVersionDestAddress} == IPv6) {
    ($sFlowSample->{NatDestAddress}) = unpack("B128", $$sFlowDatagramPacked);
    $sFlowSample->{NatDestAddress} = Net::IP::ip_bintoip($sFlowSample->{NatDestAddress},6);
    $$sFlowDatagramPacked = substr ($$sFlowDatagramPacked, 16);
  }

  else { 
    $error = "ERROR: <NatData> Unknown ip version dest address: $sFlowSample->{NatIpVersionDestAddress}";
		return ($sFlowDatagram, $sFlowSamples, \$error); 
  }
...
=head1 SYNOPSIS
  ...
  # print sflow data
  print $sFlowDatagram->{sFlowVersion};
...
a arrayreference with the sample data 
and in case of an error an error message.
...
B<optional error>
  ...
  if an error occurs decode() returns a reference to a string with the error message
